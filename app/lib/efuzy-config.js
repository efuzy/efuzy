...
const transformAssetUrls = getDevlandFile('efuzy/dist/transform-asset-urls.json')
...
/*
 * this.buildConfig           - Compiled Object from efuzy.conf.js
...
class EfuzyConfig {
      ...
      .on('change', debounce(async () => {
        ...
        try {
          ...
          if (e.message !== 'NETWORK_ERROR') {
            console.error(e)
            warn(`efuzy.conf.js has JS errors. Please fix them then save file again.\n`)
          }
  ...
  async prepare () {
    ...
    const cfg = merge({
      ...
      htmlVariables: {}
    }, this.efuzyConfigFunction(this.ctx))
  ...
  readConfig () {
    log(`Reading efuzy.conf.js`)

    if (fs.existsSync(this.filename)) {
      delete require.cache[this.filename]
      this.efuzyConfigFunction = require(this.filename)
    }
    else {
      fatal(`[FAIL] Could not load efuzy.conf.js config file`)
    }
  }

  async compile () {
    let cfg = this.efuzyConfig

    await extensionRunner.runHook('extendEfuzyConf', async hook => {
      log(`Extension(${hook.api.extId}): Extending efuzy.conf...`)
    ...
    if (this.ctx.mode.pwa) {
      cfg.pwa = merge({
        ...
        workboxOptions: {},
        manifest: {
          name: this.pkg.productName || this.pkg.name || 'Efuzy App',
          short_name: this.pkg.name || 'efuzy-pwa',
      ...
      if (cfg.pwa.manifest.icons.length === 0) {
        warn()
        warn(`PWA manifest in efuzy.conf.js > pwa > manifest is missing "icons" prop.\n`)
        process.exit(1)
      }
    ...
    if (this.ctx.mode.electron) {
      if (this.ctx.dev) {
        ...
        cfg.electron = merge({
          ...
          builder: {
            appId: 'efuzy-app',
            productName: this.pkg.productName || this.pkg.name || 'Efuzy App',
            directories: {
    ...
    // used by .efuzy entry templates
    cfg.__css = {
      efuzySrcExt: cssVariables.efuzySrcExt
    }
