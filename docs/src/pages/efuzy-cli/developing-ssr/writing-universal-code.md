...
desc: Guide on how to write code for a Efuzy server-side rendered app.
---
...
![Efuzy SSR Build System](https://cdn.efuzy.dev/img/ssr-build.png "Efuzy SSR Build System")
...
When you add a 3rd party library to your project (through a [Boot File](/efuzy-cli/boot-files)), take into consideration whether it can run on server and on client. If it needs to run only on server or only on client, then specify this in efuzy.conf.js:
...
The Efuzy CLI [PreFetch Feature](/efuzy-cli/prefetch-feature) has been created to solve this problem. Take a few moments to read about it.
