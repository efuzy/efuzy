---
title: Dark Plugin
desc: A Efuzy plugin to toggle or configure the Dark Mode state of your app.
badge: v1.3+
related:
  - /style/dark-mode
  - /style/theme-builder
...
::: tip
For a better understanding of this Efuzy plugin, please head to the Style & Identity [Dark Mode](/style/dark-mode) page.
:::
...
### Through efuzy.conf.js

You can also use `/efuzy.conf.js` to set the Dark mode status:
...
When on a SSR build:
* `import { Dark } from 'efuzy'` method of using Dark mode will not error out but it will not work (won't do anything). But you can use the other two ways (see previous section). We recommend through efuzy.conf.js.
