...
desc: Syntax of the questions that the user is going to be asked in order to configure the Efuzy App Extension.
---
...
The user's answers are stored into `/efuzy.extensions.json` (root of project folder), which should not be tampered with unless you really know what you are doing.
...
::: warning
The following is not an exhaustive list of possible types of questions and by no means it describes the full API available. Check out [Inquirer.js](https://github.com/SBoudrias/Inquirer.js#readme) for that (which is used by Efuzy CLI under the covers).
:::
...
```js
{
  ...
  default: 'A Efuzy Framework app', // optional
}
```
