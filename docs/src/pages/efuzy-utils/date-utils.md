---
title: Date Utils
desc: A set of Efuzy methods for manipulating JS Date objects without the high additional cost of dedicated libraries.
---

Efuzy provides a set of useful functions to manipulate JS Date easily in most use cases, without the high additional cost of integrating dedicated libraries like Momentjs.

Most Efuzy date functions take as parameter either a Unix timestamp or a String representing a date which needs to be parsable by the native JS [Date constructor](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date). Some examples: `1497159857411`, `Sun Jun 11 2017 08:44:42 GMT+0300`, `2017-06-16`.
...
::: tip
Efuzy date utils includes tree shaking, except for the UMD version.
:::

You will notice all examples import `date` Object from Efuzy. However, if you need only one method from it, then you can use ES6 destructuring to help Tree Shaking embed only that method and not all of `date`.

Example with `addToDate()`:

```js
// we import all of `date`
import { date } from 'efuzy'
...
::: tip
For usage with the UMD build see [here](/start/umd#Efuzy-Global-Object).
:::
...
Using locale set by current Efuzy language pack, this allows you to parse any string into a date object based on the format passed:
