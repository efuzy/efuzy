...
desc: How to publish a Efuzy hybrid mobile app with Capacitor to Google Play Store and to Apple App Store.
---
...
```bash
$ efuzy build -m capacitor -T android
```
...
In order for the Google Play Store to accept updated APKs, you’ll need to bump the app version (from `/package.json` or from `/efuzy.conf.js > capacitor > version`, then rebuild the app for release.
