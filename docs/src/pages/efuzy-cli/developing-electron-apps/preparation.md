...
desc: How to add Electron mode into a Efuzy app.
---
Before we dive in to the actual development, we need to do some preparation work.

## 1. Add Efuzy Electron Mode
In order to develop/build a Efuzy Electron app, we need to add the Electron mode to our Efuzy project. What this does is that it yarn installs some Electron packages and creates `/src-electron` folder.
```bash
$ efuzy mode add electron
```
...
At this point things should successfully install, but if not then you will need a clean installation of Visual Studio. Please note that these are not problems with Efuzy, but they are related to NPM and Windows.

## 2. Start Developing
If you want to jump right in and start developing, you can skip the previous step with "efuzy mode" command and issue:
