---
title: DOM Utils
desc: A set of Efuzy methods for DOM elements which helps you in retrieving the offset on screen viewport, getting and setting styles and waiting for the DOM to be ready.
---

### Helping Tree-Shake
You will notice all examples import different parts of Efuzy. However, if you need only one specific util method, then you can use ES6 destructuring to help Tree Shaking embed only that method and not all around it.

Example with `dom` utils:
```js
import { dom } from 'efuzy'
const { offset } = dom
