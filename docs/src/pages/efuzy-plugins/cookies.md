---
title: Cookies
desc: A Efuzy plugin which manages browser cookies over the standardized 'document.cookie', making it easy to read and write cookies even with SSR apps.
---
...
## Notes on SSR
When building for SSR, use only the `$q.cookies` form. If you need to use the `import { Cookies } from 'efuzy'`, then you'll need to do it like this:

```js
import { Cookies } from 'efuzy'
...
The `ssrContext` is available in [boot files](/efuzy-cli/boot-files) or [preFetch feature](/efuzy-cli/prefetch-feature) where it is supplied as parameter.
...
// Following formats require Efuzy v1.7+:
...
::: tip
If using Efuzy CLI and [on dev mode](/efuzy-cli/efuzy-conf-js#Property%3A-devServer), you can enable HTTPS through efuzy.conf.js > devServer > https: true.
:::
...
Raw string for other cookie options. To be used as a last resort for possible newer props that are currently not yet implemented in Efuzy.
