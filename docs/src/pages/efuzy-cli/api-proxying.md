...
desc: How to use an API proxy with the Efuzy dev server.
related:
  - /efuzy-cli/efuzy-conf-js
---
When integrating a project folder (created by Efuzy CLI) with an existing backend, a common need is to access the backend API when using the dev server. To achieve this, we can run the dev server and the API backend side-by-side (or remotely), and let the dev server proxy all API requests to the actual backend.
...
To configure the proxy rules, edit `/efuzy.conf.js` in `devServer.proxy`. You should refer to [Webpack Dev Server Proxy](https://webpack.js.org/configuration/dev-server/#devserver-proxy) docs for detailed usage. But here's a simple example:
