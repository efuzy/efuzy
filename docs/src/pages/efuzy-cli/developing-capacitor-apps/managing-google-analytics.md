...
desc: How to use analytics in a Efuzy hybrid mobile app with Capacitor.
---
...
Follow this guide to implement Google Analytics into your Capacitor powered Efuzy App.

You may also want to read this great tutorial: [Google Tag Manager and Analytics Setup for an SPA Website](https://jannerantala.com/tutorials/efuzy-framework-google-tag-manager-and-analytics-setup-for-an-spa-website/)
...
```
Finally we register the app boot file in `/efuzy.conf.js`. We can do so only for Capacitor wrapped apps if we want:
```
boot: [
