...
badge: "@efuzy/app v1.6+"
desc: How to enable support for TypeScript in a Efuzy app.
related:
...
::: tip
The following steps are only required when you **have not** selected TypeScript support when creating a fresh Efuzy project. If you selected the TS option on project creation, TypeScript support is already enabled.
:::

## Installation of TypeScript Support

In order to support TypeScript, you'll need to edit `/efuzy.conf.js`:
...
```json
{
  "extends": "@efuzy/app/tsconfig-preset",
  "compilerOptions": {
    "baseUrl": "."
  }
}
...
Behind the curtains, Efuzy uses `ts-loader` and `fork-ts-checker-webpack-plugin` (provided by `@efuzy/app` package) to manage TS files. If you ever need to provide a custom configuration for these libs you can do so by making `supportTS` property like so:
...
::: tip
TypeScript Linting is really slow due to type-checking overhead, we suggest you to disable Webpack lint extension into `efuzy.conf.js` for dev builds.
:::

If you setup TypeScript linting and want `fork-ts-checker-webpack-plugin` (provided by `@efuzy/app` package) to take it into account then you should make use of `tsCheckerConfig` property:
