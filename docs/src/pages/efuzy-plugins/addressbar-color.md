...
desc: A Efuzy plugin for changing the addressbar color on newer mobile browsers.
---
...
![Mobile Addressbar Coloring](https://cdn.efuzy.dev/img/mobile-address-bar-colors.jpg "Mobile Addressbar Coloring")
...
We create boot file to initialize its usage: `$ efuzy new boot addressbar-color`. A file is created (`/src/boot/addressbar-color.js`). We edit it:

```js
// file: /src/boot/addressbar-color.js
import { AddressbarColor } from 'efuzy'
...
We then have to tell efuzy to use this boot file we just created. To do this we edit the boot section of the efuzy config:
```js
// file: /efuzy.conf.js
return {
  boot: [
