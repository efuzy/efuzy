...
desc: How to prefetch data and initialize your Vuex store, validate the route and redirect to another page in a Efuzy app.
related:
  - /efuzy-cli/efuzy-conf-js
---
The PreFetch is a feature (**only available when using Efuzy CLI**) which allows the components picked up by Vue Router (defined in `/src/router/routes.js`) to:
* pre-fetch data
* validate the route
...
**It is designed to work with all Efuzy modes** (SPA, PWA, SSR, Cordova, Electron), but it is especially useful for SSR builds.
...
| `/shop/product/shoes` | ShopProduct | Efuzy notices the same component is already rendered, but the route has been updated and it has route params, so it calls the hook again. |
...
export default {
  ...
  preFetch ({ store, currentRoute, previousRoute, redirect, ssrContext, urlPath, publicPath }) {
    // urlPath and publicPath requires @efuzy/app v2+
...
// an initialized Efuzy project splits every component of a Vuex module
...
```js
import { preFetch } from 'efuzy/wrappers'
...
## Loading State
A good UX includes notifying the user that something is being worked on in the background while he/she waits for the page to be ready. Efuzy CLI offers two options for this out of the box.

### LoadingBar
When you add Efuzy [LoadingBar](/efuzy-plugins/loading-bar) plugin to your app, Efuzy CLI will use it while it runs the preFetch hooks by default.

### Loading
There's also the possibility to use Efuzy [Loading](/efuzy-plugins/loading) plugin. Here's an example:

```js
// a route .vue component
import { Loading } from 'efuzy'
