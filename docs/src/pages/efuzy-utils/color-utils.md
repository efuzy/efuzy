---
title: Color Utils
desc: A set of Efuzy methods for changing app brand colors and manipulating color strings.
---

Efuzy provides a set of useful functions to manipulate colors easily in most use cases, without the high additional cost of integrating dedicated libraries.

### Helping Tree-Shake
You will notice all examples import `colors` Object from Efuzy. However, if you need only one method from it, then you can use ES6 destructuring to help Tree Shaking embed only that method and not all of `colors`.
...
### Helper - getBrand
Efuzy offers a helper function for getting custom colors in the `colors` utils: `getBrand(colorName[, element])`
