...
desc: How to manage your Browser Extensions with Efuzy CLI.
---
...
You do not need a new Efuzy App per BEX type above as a single Efuzy Application can run in **all** of the instances above. You can find out more about these in the [types section](/efuzy-cli/developing-browser-extensions/types-of-bex).
...
When you create your Efuzy BEX, the manifest file is already configured to add the basic properties you will need in order to run your BEX. This includes default background scripts, content scripts and a css file which is injected in the context of the web page the BEX is running on.
...
Any styles you want to be made available to your web page (not your Efuzy App) should be included in `src-bex/css/content-css.css` as this file ia automatically injected into the `manifest.json` file.
...
In a Efuzy BEX, you are provided with `background-hook.js`, `content-hook.js` and `dom-hook.js`. These files are designed to give you access to a bridge which closes the gap in communication with each layer of a BEX. We will explore them in more detail in the [next section](/efuzy-cli/developing-browser-extensions/bex-communication).
