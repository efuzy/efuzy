...
desc: A set of Efuzy methods related to scrolling, like getting scroll target or changing the scroll position of a page.
---
...
This method searches for a parent DOM element which has one of the `scroll` or `overflow-auto` Efuzy CSS Helper class attached to it. If none is found, then it considers that the scrolling takes place on the document itself.
