...
desc: What you need to do before developing a Efuzy hybrid mobile app with Capacitor.
---
...
## 2. Add Capacitor Efuzy Mode

In order to develop/build a Mobile app, we need to add the Capacitor mode to our Efuzy project. This will use the Capacitor CLI to generate a Capacitor project in `/src-capacitor` folder.

```bash
$ efuzy mode add capacitor
```
...
```bash
$ efuzy dev -m capacitor -T [android|ios]
```
