...
export default {
  ...
  methods: {
    queryReleases (page = 1) {
      ...
      const self = this,
        xhrEfuzy = new XMLHttpRequest()

      xhrEfuzy.addEventListener('load', function () {
        self.loading = false
      ...
      xhrEfuzy.addEventListener('error', () => {
        this.error = true
      })

      xhrEfuzy.open('GET', `https://api.github.com/repos/efuzy/efuzy/releases?page=${page}&per_page=100`)
      xhrEfuzy.send()
    }
  }
}
