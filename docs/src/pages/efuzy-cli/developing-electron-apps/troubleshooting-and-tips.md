...
desc: Tips and tricks for a Efuzy desktop app with Electron.
---
...
::: warning
Accessing `this.$q.electron` requires that the [Node Integration](/efuzy-cli/developing-electron-apps/node-integration) is kept turned "on".
:::
...
Some Chrome DevTools Extensions do not play well with Windows Dark Theme on electron 6+. Efuzy offers a workaround in the default `electron-main.js`, that removes the `DevTools Extensions` before starting the application.
