...
desc: How to manage the Vuex Store in a Efuzy App.
---
...
We won't go into details on how to configure or use Vuex since it has great docs. Instead we'll just show you what the folder structure looks like when using it on a Efuzy project.
...
By default, if you choose to use Vuex when you create a project folder with Efuzy CLI, it will set you up on using Vuex modules. Each sub-folder of `/src/store` represents a Vuex Module.
...
Adding a Vuex Module is made easy by Efuzy CLI through the `$ efuzy new` command.

```bash
$ efuzy new store <store_name>
```
...
Let's say that you want to create a "showcase" Vuex Module. You issue `$ efuzy new store showcase`. You then notice the newly created `/src/store/showcase` folder, which holds the following files:
...
You can take advantage of the [PreFetch Feature](/efuzy-cli/prefetch-feature#Store-Code-Splitting) to code-split Vuex modules.
