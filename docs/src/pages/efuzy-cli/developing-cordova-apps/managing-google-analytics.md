...
desc: How to use analytics in a Efuzy hybrid mobile app with Cordova.
---
...
Follow this guide to implement Google Analytics into your Cordova powered Efuzy App.

You may also want to read these great tutorials:
- [Google Tag Manager and Analytics Setup for an SPA Website](https://jannerantala.com/tutorials/efuzy-framework-google-tag-manager-and-analytics-setup-for-an-spa-website/)
- [Google Analytics Setup for a Cordova App](https://jannerantala.com/tutorials/efuzy-framework-google-analytics-setup-for-cordova-app/)
...
## Prerequisites
* Make sure all your routes have a name and path parameter specified. Otherwise, they cannot be posted to the `ga.logPage` function. Please refer to [Routing](/efuzy-cli/routing) for more info on routing.
...
```bash
$ efuzy new boot google-analytics
```
...
```
Finally we register the app boot file in `/efuzy.conf.js`. We can do so only for Cordova wrapped apps if we want:
```
boot: [
