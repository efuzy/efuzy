...
desc: Tips and tricks for a Efuzy hybrid mobile app with Cordova.
---
...
![Android Remote Debugging](https://cdn.efuzy.dev/img/remote-debug.png "Android Remote Debugging")
![Android Remote Debugging](https://cdn.efuzy.dev/img/remote-debug-2.png "Android Remote Debugging")
...
<img src="https://cdn.efuzy.dev/img/gradle-upgrade-notice.png" alt="Gradle upgrade" class="q-mb-md fit rounded-borders" style="max-width: 350px">

If you encounter any IDE errors then click on File > Invalidate caches and restart.

<img src="https://cdn.efuzy.dev/img/gradle-invalidate-cache.png" alt="Gradle upgrade" class="fit rounded-borders" style="max-width: 350px">
...
There are intermediate states to help with debugging, between `efuzy dev` and distributing a completed app. If your app works fine on `efuzy dev` but is not running properly after `efuzy build`, you have two options:
...
* open Android Studio and watch the Logcat
  ...
  * Note: this should be for experienced Android developers only. If your app is not functioning properly, it is much more likely that `efuzy dev` or `cordova run` can reveal the issue.

::: danger Important!
If you find a bug using one of the above methods, do not edit the output files (probably the `www` folder) directly, as they will soon be overwritten. Go back to your efuzy source, fix the bug, then re-run `efuzy build`.
:::

## iOS Tips

### Device type not found
If you get this error while running `$ efuzy dev -m cordova -T ios`:
...
```bash
$ efuzy dev -m cordova -T ios -e iPhone-X,12.2
# or whith older versions of Cordova CLI installed on your machine:
$ efuzy dev -m cordova -T ios -e iPhone-X,com.apple.CoreSimulator.SimRuntime.iOS-12-2
```
...
Efuzy has support for these CSS safe-areas by default in QHeader/QFooter and Notify. However it's important to always check your Cordova build on several models to see if all cases of your app are dealing with the safe areas correctly.
...
In order to make sure these are only added when opened on mobile via the Cordova build, you can check for the CSS class `.cordova` which is automatically added to the body by Efuzy. Example:
